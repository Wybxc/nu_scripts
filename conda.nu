...
export def list [
  --name(-n): string@'nu-complete conda envs' = 'base'  # name of the environment
] {
  ^conda list --name $name --json | from json
  | select name version platform channel
  | sort-by name
}
